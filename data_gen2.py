...
class DataGen2:
    ...
    def generate_multi(self, corpus, batches=-1):
        batch_size = self.batch_size
        seq_length = self.seuqnce_length
        to_read = batch_size + seq_length
        tex_data_file = open(corpus, mode='r', encoding='utf-8')
        ...
        while True:
            ...
            for b in range(batch_size):
                ...
                taregt = seq[seq_length + b]
                num_encoded = self.encode_text_to_num(text)
                hots = one_hot_encode2(num_encoded, input_size)
                batch_x[b] = hots
                c_output = one_hot_encode(
                    self.char2tup[taregt][0], self.n_consonants)
                v_output = one_hot_encode(
                    self.char2tup[taregt][1], self.n_vowels)
